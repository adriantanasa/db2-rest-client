...
const debugInfo = require('debug')('db2-on-cloud-rest:info');
...
class Db2RestClient {
    ...
    async request(type = REQ_TYPE_DEFAULT, extraOptions = {}, authRequired = true) {
        ...
        debugNotice('[request] result:', data);
        return data;
    }
    ...
    async requestPolling(type = REQ_TYPE_DEFAULT, extraOptions = {}, success = {status: 'completed'},
        ...
        if (tryCount < poolingMaxRetries) {
            debugNotice('[requestPolling] plan next check in %s ms / tick %s', poolingTimeout, tryCount);
